...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        predicted_label = labels.get(predicted_prob[0].argmax(), "Unknown")
        color = (255, 0, 255)
        stroke = 2
        cv2.putText(frame, f'({predicted_label})', (x, y - 8),
                    font, 1, color, stroke, cv2.LINE_AA)

        found_trained_face = True

    if not found_trained_face:
        font = cv2.FONT_HERSHEY_SIMPLEX
        color = (255, 0, 255)
        stroke = 2
